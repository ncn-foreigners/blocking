...
+ `cis` object has `r nrow(census)` rows and `r ncol(census)` columns.
...
We need to create new columns that concatenate variables from `PERNAME1` to `ENUMPC`. In the first step we replace `NA`s with `''`. 
