...
#' applies approximate nearest neighbour (ANN) algorithms via the \link[rnndescent]{rnndescent}, \link[RcppHNSW]{RcppHNSW}, \link[RcppAnnoy]{RcppAnnoy}, \link[mlpack]{mlpack} and \pkg{duckdb} packages,
...
#' @param ann algorithm to be used for searching for ann (possible, \code{c("nnd", "hnsw", "annoy", "lsh", "kd", "duckdb")}, default \code{"nnd"} which corresponds to nearest neighbour descent method),
...
blocking <- function(x,
                     ...
                     on = NULL,
                     on_blocking = NULL,
                     ann = c("nnd", "hnsw", "annoy", "lsh", "kd", "duckdb"),
  ...
  if (missing(distance)) distance <- switch(ann,
                                            ...
                                            "lsh" = NULL,
                                            "kd" = NULL,
                                            "duckdb" = if (control_ann$duckdb$engine == "faiss") "euclidean" else "cosine")
  ...
  if ((ann == "duckdb") && !is.null(ann_write)) {
    stop("`ann_write` is not supported for the DuckDB backend.", call. = FALSE)
  }
  ...
  if (deduplication && (NROW(x) <= 1L)) {
    return(structure(
      list(
        result = data.table(x = integer(),
                            y = integer(),
                            block = integer(),
                            dist = numeric()),
        method = ann,
  ...
  if (nrow(x_df)) {
    ...
    x_df[, `:=`(x = as.integer(x), y = as.integer(y), block = integer())]
  }
