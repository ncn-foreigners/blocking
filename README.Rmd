...
+ [duckdb](https://cran.r-project.org/package=duckdb) with the DuckDB `vss` extension.
...
If you want to use `ann = "duckdb"`, install or update the DuckDB
`vss` extension once before calling `blocking()`:
...
DuckDB can also be used as the ANN backend. The helper below checks that
the `vss` extension is installed and reports whether DuckDB exposes the
batched `HNSW_INDEX_JOIN` optimization:
