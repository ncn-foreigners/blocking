...
duckdb_load_extension <- function(con, control_duckdb) {
  ...
  install_sql <- switch(engine,
                        "vss" = "INSTALL vss",
                        "faiss" = "INSTALL faiss FROM community")
  ...
  tryCatch({
    if (isTRUE(control_duckdb$install_extension)) {
      DBI::dbExecute(con, install_sql)
    }
    DBI::dbExecute(con, load_sql)
  }, error = function(e) {
    stop(sprintf("DuckDB `%s` extension could not be loaded: %s",
                 engine,
                 conditionMessage(e)),
         call. = FALSE)
  })
...
duckdb_prepare_text_backend <- function(con,
  ...
  DBI::dbExecute(con,
                 sprintf("CREATE TABLE x_input AS SELECT id, %s AS txt FROM x_raw",
                         norm_expr))
  DBI::dbExecute(con,
                 sprintf("CREATE TABLE y_input AS SELECT id, %s AS txt FROM y_raw",
                         norm_expr))
...
method_duckdb <- function(x,
  ...
  stopifnot(
    "DuckDB backend currently supports only character vectors and `representation = \"shingles\"`" =
  ...
  prepared <- duckdb_prepare_text_backend(con = con,
                                          x = x,
                                          y = y,
